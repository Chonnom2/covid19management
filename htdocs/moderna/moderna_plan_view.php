...
<div style="position: absolute; top:8%; left:82%;"><button onclick="location.href='moderna_plan_search.php'" style="font-size:15px;width:100px;height:45%">계약검색</button></div>

<br><br>
<?php
  // 검색어에 따라 다른 튜플 개수 셈(페이징 처리를 위해 필요)
  $sql = "SELECT * FROM `moderna_plan_count`";
  ...
  $sql = "SELECT * FROM (SELECT `plan num`,`contract_num`,`start date`,`finish date`,`quantity`,`plant_code`,`company`  FROM `production plan` AS PLAN JOIN `plant`AS PLANT ON PLAN.`plant_code` = PLANT.`plant code`) AS m WHERE m.`company` = '모더나' ORDER BY `plan num` DESC LIMIT $page_start, $list";
  $ret = mysqli_query($con, $sql);
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_plan_view.php?page=1'> 처음 </a>";
    echo "<a href='moderna_plan_view.php?page=$pre'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_plan_view.php?page=$i'> $i </a>";
      }
    }
    echo "<a href='moderna_plan_view.php?page=$next'> 다음 </a>";
    echo "<a href='moderna_plan_view.php?page=$total_page'> 마지막 </a>";
