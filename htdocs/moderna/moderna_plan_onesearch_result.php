...
<?php
  ...
  {
      $search_array = array($_POST["plan_num"],$_POST["connum"],$_POST["sdate"],$_POST["fdate"],$_POST["quantity"],$_POST["plantcode"]);     
      for($j = 0; $j < 6; ++$j)
      {
  ...
  if($j == 6)
  {
    ...
    echo "<a href= 'moderna_plan_search.php'> 되돌아가기 </a>";
    exit();
  }
...
<div style="position: absolute; top:8%; left:91%;"><button onclick="location.href='moderna_plan_add.php'" style="font-size:15px;width:100px;height:45%">계획추가</button></div>
<div style="position: absolute; top:8%; left:82%;"><button onclick="location.href='moderna_plan_search.php'" style="font-size:15px;width:100px;height:45%">계획검색</button></div>
<br><br>
<?php
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `plan num` = $search_array[0]";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `contract_num` = $search_array[1]";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `start date` = '$search_array[2]'";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `finish date` = '$search_array[3]'";
        break;        
    case 4:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `quantity` = $search_array[4]";
        break; 
    case 5:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `plant_code` = '$search_array[5]'";
        break;
  } 
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `plan num` = $search_array[0] ORDER BY `plan num` DESC LIMIT $page_start, $list";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `contract_num` = $search_array[1] ORDER BY `plan num` DESC LIMIT $page_start, $list";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `start date` = '$search_array[2]' ORDER BY `plan num` DESC LIMIT $page_start, $list";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `finish date` = '$search_array[3]' ORDER BY `plan num` DESC LIMIT $page_start, $list";
        break;        
    case 4:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `quantity` = $search_array[4] ORDER BY `plan num` DESC LIMIT $page_start, $list";
        break; 
    case 5:
        $sql = "SELECT * FROM `covid-19`.`moderna_plan` WHERE `plant_code` = '$search_array[5]' ORDER BY `plan num` DESC LIMIT $page_start, $list";
        break;
  ...
  while($row = mysqli_fetch_array($ret)) {
    ...
    echo "<TD><center>", "<a href = 'moderna_contract_information.php?contract_num=".$row['contract_num']. "'>",$row['contract_num'],"</TD>";
    echo "<TD><center>", $row['start date'], "</TD>";
    echo "<TD><center>", $row['finish date'], "</TD>";
    echo "<TD><center>", number_format($row['quantity']), "</TD>";
    echo "<TD><center>", "<a href = 'moderna_plant_information.php?plant_code=".$row['plant_code']. "'>",$row['plant_code'],"</TD>";
    echo "<TD><center>", "<a href= 'moderna_plan_finish.php?plan_num=" .$row['plan num']. "'>확인</a></TD>";
    echo "<TD><center>", "<a href= 'moderna_plan_update.php?plan_num=" .$row['plan num']. "'>수정</a></TD>";
    echo "<TD><center>", "<a href= 'moderna_plan_delete.php?plan_num=" .$row['plan num']. "'>삭제</a></TD>";
    echo "</TR>";
    $brcount++;
  }
  echo "</TABLE><br>";
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_plan_onesearch_result.php?page=1&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&j=$j'> 처음 </a>";
    echo "<a href='moderna_plan_onesearch_result.php?page=$pre&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&j=$j'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_plan_onesearch_result.php?page=$i&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&j=$j'> $i </a>";
        
      }
    }
    echo "<a href='moderna_plan_onesearch_result.php?page=$next&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&j=$j'> 다음 </a>";
    echo "<a href='moderna_plan_onesearch_result.php?page=$total_page&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&j=$j'> 마지막 </a>";
