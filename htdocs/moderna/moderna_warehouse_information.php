...
$sql = "SELECT * FROM `covid-19`.`warehouse` WHERE `warehouse code` = '$warehousecode'";
...
echo "<TR>";
echo "<TH>창고 코드</TH><TH>창고 주소</TH><TH>창고 연락처</TH><TH>창고 용량</TH><TH>수정</TH><TH>삭제</TH>";
echo "</TR>";
echo "<TR>";
echo "<TD><center>", $row['warehouse code'], "</TD>";
echo "<TD><center>", $row['address'], "</TD>";
echo "<TD><center>", $row['phone'],"</TD>";
echo "<TD><center>", $row['capacity'],"</TD>";
echo "<TD><center>", "<a href= 'moderna_warehouse_update.php?lot_num=" .$row['warehouse code']. "'>수정</a></TD>";
echo "<TD><center>", "<a href= 'moderna_warehouse_delete.php?lot_num=" .$row['warehouse code']. "'>삭제</a></TD>";
echo "</TR>";
mysqli_close($con);
echo "</TABLE><br>";
