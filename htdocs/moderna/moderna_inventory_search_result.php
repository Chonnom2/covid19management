...
// 페이지 정보 불러오기
  ...
  {
      $search_array = array($_POST["s_inventory_num"],$_POST["f_inventory_num"],$_POST["lotnum"],$_POST["warehousecode"],
                            $_POST["s_quantity"],$_POST["f_quantity"]);
      for($i = 0; $i < 6; ++$i)
      {
          if(empty($search_array[$i]))
          {
              if($i == 0 || $i == 4) {$search_array[$i] = 0;}
              else if($i == 1 ||$i == 5) {$search_array[$i] = INTMAX;}
              else if($i == 2) {$j = 0;} 
              else if($i == 3) {$k = 0;}
...
<div style="position: absolute; top:8%; left:91%;"><button onclick="location.href='moderna_inventory_add.php'" style="font-size:15px;width:100px;height:45%">재고추가</button></div>
<div style="position: absolute; top:8%; left:82%;"><button onclick="location.href='moderna_inventory_search.php'" style="font-size:15px;width:100px;height:45%">재고검색</button></div>
<br><br>
<?php
  ...
  {
      case 0:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND  `quantity` BETWEEN $search_array[4] AND $search_array[5]";
        break;
      case 1:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND `warehouse_code` = '$search_array[3]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5]";
        break;
      case 2:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND `lot number` = '$search_array[2]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5]";
        break;
      case 3:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND `lot number` = '$search_array[2]'AND `warehouse_code` = '$search_array[3]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5]";
        break;
  } 
  ...
  switch($b)
  {
      case 0:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND  `quantity` BETWEEN $search_array[4] AND $search_array[5] ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;
      case 1:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND `warehouse_code` = '$search_array[3]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5] ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;
      case 2:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND `lot number` = '$search_array[2]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5] ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;
      case 3:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` BETWEEN $search_array[0] AND $search_array[1] 
        AND `lot number` = '$search_array[2]'AND `warehouse_code` = '$search_array[3]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5] ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;
  }
  $ret = mysqli_query($con, $sql);
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_inventory_search_result.php?page=1&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&b=$b'> 처음 </a>";
    echo "<a href='moderna_inventory_search_result.php?page=$pre&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&b=$b'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_inventory_search_result.php?page=$i&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&b=$b'> $i </a>";
        
      }
    }
    echo "<a href='moderna_inventory_search_result.php?page=$next&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&b=$b'> 다음 </a>";
    echo "<a href='moderna_inventory_search_result.php?page=$total_page&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&b=$b'> 마지막 </a>";
