...
<?php
  ...
  $sql = "SELECT * FROM `moderna_inventory_count`";
  ...
  $sql = "SELECT * FROM `covid-19`.`moderna_inventory` ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
  $ret = mysqli_query($con, $sql);
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_inventory_view.php?page=1'> 처음 </a>";
    echo "<a href='moderna_inventory_view.php?page=$pre'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_inventory_view.php?page=$i'> $i </a>";
      }
    }
    echo "<a href='moderna_inventory_view.php?page=$next'> 다음 </a>";
    echo "<a href='moderna_inventory_view.php?page=$total_page'> 마지막 </a>";
