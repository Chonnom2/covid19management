...
// 페이지 정보 불러오기
  ...
  {
      $search_array = array($_POST["s_contract_num"],$_POST["f_contract_num"],$_POST["s_date"],$_POST["f_date"],
                      $_POST["s_quantity"],$_POST["f_quantity"],$_POST["s_uprice"],$_POST["f_uprice"],$_POST["s_duedate"],$_POST["f_duedate"]);
      for($i = 0; $i < 10; ++$i)
      {
          if(empty($search_array[$i]))
          {
              if($i == 0 || $i == 4 || $i == 6) {$search_array[$i] = 0;}
              else if($i == 1 || $i == 5 || $i == 7) {$search_array[$i] = INTMAX;}
              else if($i == 2 || $i == 8) {$search_array[$i] = "0000-01-01";}
              else {$search_array[$i] = "9999-12-31";}
          } 
      }
  }
    $con=mysqli_connect("localhost", "root", "1234", "covid-19") or die("MySQL 접속 실패");
?>
<div style="position: absolute; top:8%; left:45%;font-size:25px;width:15%;height:50%;">계약 조회 결과</div>
...
<?php
  ...
  $sql = "SELECT * FROM `covid-19`.`contract` WHERE `contract num` BETWEEN $search_array[0] AND $search_array[1] 
          AND `date` BETWEEN '$search_array[2]' AND '$search_array[3]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5]
          AND `unit price` BETWEEN $search_array[6] AND $search_array[7] AND `due date` BETWEEN '$search_array[8]' AND '$search_array[9]'AND `supplies` = '모더나'";
  $ret = mysqli_query($con, $sql);
  if($ret){
    $count = mysqli_num_rows($ret);
    if($count == 0){
        echo $_GET['s_contract_num']." 해당 계약 넘버의 계약이 없음!!"."<br>";
  ...
  $sql = "SELECT * FROM `covid-19`.`contract` WHERE `contract num` BETWEEN $search_array[0] AND $search_array[1] 
          AND `date` BETWEEN '$search_array[2]' AND '$search_array[3]' AND `quantity` BETWEEN $search_array[4] AND $search_array[5]
          AND `unit price` BETWEEN $search_array[6] AND $search_array[7] AND `due date` BETWEEN '$search_array[8]' AND '$search_array[9]'AND `supplies` = '모더나'
          ORDER BY `contract num` DESC LIMIT $page_start, $list";
  $ret = mysqli_query($con, $sql);
  mysqli_close($con);
  // 표만들기
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_contract_search_result.php?page=1&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&arr[]=$search_array[6]&arr[]=$search_array[7]&arr[]=$search_array[8]&arr[]=$search_array[9]'> 처음 </a>";
    echo "<a href='moderna_contract_search_result.php?page=$pre&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&arr[]=$search_array[6]&arr[]=$search_array[7]&arr[]=$search_array[8]&arr[]=$search_array[9]'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_contract_search_result.php?page=$i&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&arr[]=$search_array[6]&arr[]=$search_array[7]&arr[]=$search_array[8]&arr[]=$search_array[9]'> $i </a>";
        
      }
    }
    echo "<a href='moderna_contract_search_result.php?page=$next&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&arr[]=$search_array[6]&arr[]=$search_array[7]&arr[]=$search_array[8]&arr[]=$search_array[9]'> 다음 </a>";
    echo "<a href='moderna_contract_search_result.php?page=$total_page&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&arr[]=$search_array[5]&arr[]=$search_array[6]&arr[]=$search_array[7]&arr[]=$search_array[8]&arr[]=$search_array[9]'> 마지막 </a>";
