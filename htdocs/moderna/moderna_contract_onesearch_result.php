...
<?php
  ...
  {
      $search_array = array($_POST["contract_num"],$_POST["date"],$_POST["quantity"],$_POST["uprice"],$_POST["duedate"]);     
      for($j = 0; $j < 5; ++$j)
      {
  ...
  if($j == 5)
  {
    ...
    echo "<a href= 'moderna_contract_search.php'> 되돌아가기 </a>";
    exit();
  }
...
<div style="position: absolute; top:8%; left:45%;font-size:25px;width:15%;height:50%;">계약 검색 결과</div>
<div style="position: absolute; top:8%; left:91%;"><button onclick="location.href='moderna_contract_add.php'" style="font-size:15px;width:100px;height:45%">계약추가</button></div>
<div style="position: absolute; top:8%; left:82%;"><button onclick="location.href='moderna_contract_search.php'" style="font-size:15px;width:100px;height:45%">계약검색</button></div>
...
<?php
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `contract num` = $search_array[0] AND `supplies` = '모더나' ";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `date` = '$search_array[1]' AND `supplies` = '모더나'";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `quantity` = $search_array[2] AND `supplies` = '모더나'";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `unit price` = $search_array[3] AND `supplies` = '모더나'";
        break;        
    case 4:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `due date` = '$search_array[4]' AND `supplies` = '모더나'";
        break; 
  } 
  ...
  if($ret){
    ...
    if($total == 0){
        echo $_GET['contract_num']." 해당 계약 넘버의 계약이 없음!!"."<br>";
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `contract num` = $search_array[0] AND `supplies` = '모더나' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `date` = '$search_array[1]' AND `supplies` = '모더나' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `quantity` = $search_array[2] AND `supplies` = '모더나' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `unit price` = $search_array[3] AND `supplies` = '모더나' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;        
    case 4:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `due date` = '$search_array[4]' AND `supplies` = '모더나' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break; 
  } 
  $ret = mysqli_query($con, $sql);
  mysqli_close($con);
  ...
  echo "<TR>";
  echo "<TH><CENTER>계약번호</CENTER></TH><TH><CENTER>계약일자</CENTER></TH><TH><CENTER>수량</CENTER></TH><TH><CENTER>단위 가격</CENTER></TH><TH><CENTER>납기 기한</CENTER></TH><TH><CENTER>총 금액</CENTER></TH><TH><CENTER>생산계획 생성</CENTER></TH><TH><CENTER>수정</CENTER></TH><TH><CENTER>삭제</CENTER></TH>";
  echo "</TR></thead>";
  $unit = " $";
  $brcount = 0;
  while($row = mysqli_fetch_array($ret)) {
    ...
    echo "<TD><center>", "<a href= 'moderna_contract_toplan.php?contract_num=" .$row['contract num']. "'>생성</a></TD>";
    echo "<TD><center>", "<a href= 'moderna_contract_update.php?contract_num=" .$row['contract num']. "'>수정</a></TD>";
    echo "<TD><center>", "<a href= 'moderna_contract_delete.php?contract_num=" .$row['contract num']. "'>삭제</a></TD>";
    echo "</TR>";
    $brcount++;
  }
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_contract_onesearch_result.php?page=1&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 처음 </a>";
    echo "<a href='moderna_contract_onesearch_result.php?page=$pre&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_contract_onesearch_result.php?page=$i&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> $i </a>";
        
      }
    }
    echo "<a href='moderna_contract_onesearch_result.php?page=$next&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 다음 </a>";
    echo "<a href='moderna_contract_onesearch_result.php?page=$total_page&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 마지막 </a>";
