...
<?php
  ...
  if($j == 4)
  {
    ...
    echo "<a href= 'moderna_inventory_search.php'> 되돌아가기 </a>";
    exit();
  }
...
<?php
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` = $search_array[0]";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `lot number` = '$search_array[1]'";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `warehouse_code` = '$search_array[2]'";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `quantity` = $search_array[3]";
        break;        
  } 
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `inventory_num` = $search_array[0] ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `lot number` = '$search_array[1]' ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `warehouse_code` = '$search_array[2]' ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`moderna_inventory` WHERE `quantity` = $search_array[3] ORDER BY `inventory_num` DESC LIMIT $page_start, $list";
        break;        
  } 
  $ret = mysqli_query($con, $sql);
  if($ret){
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='moderna_inventory_onesearch_result.php?page=1&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&j=$j'> 처음 </a>";
    echo "<a href='moderna_inventory_onesearch_result.php?page=$pre&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&j=$j'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'moderna_inventory_onesearch_result.php?page=$i&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&j=$j'> $i </a>";
        
      }
    }
    echo "<a href='moderna_inventory_onesearch_result.php?page=$next&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&j=$j'> 다음 </a>";
    echo "<a href='moderna_inventory_onesearch_result.php?page=$total_page&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&j=$j'> 마지막 </a>";
