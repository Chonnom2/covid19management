...
$lotnum = $_GET['lot_num'];
$sql = "SELECT * FROM `covid-19`.`vaccine information` WHERE `lot num` = '$lotnum'";
...
echo "<TR>";
echo "<TH>로트 번호</TH><TH>유효 기간</TH><TH>생산 계획 번호</TH><TH>생산 공장 코드</TH><TH>수정</TH><TH>삭제</TH>";
echo "</TR>";
echo "<TR>";
echo "<TD><center>", $row['lot num'], "</TD>";
echo "<TD><center>", $row['shelf life'], "</TD>";
echo "<TD><center>", "<a href = 'moderna_plan_information.php?plan_num=".$row['plan_num']. "'>",$row['plan_num'],"</TD>";
echo "<TD><center>", "<a href = 'moderna_plant_information.php?plant_code=".$row['prod_plant_code']. "'>",$row['prod_plant_code'],"</TD>";
echo "<TD><center>", "<a href= 'moderna_vaccineinform_update.php?lot_num=" .$row['lot num']. "'>수정</a></TD>";
echo "<TD><center>", "<a href= 'moderna_vaccineinform_delete.php?lot_num=" .$row['lot num']. "'>삭제</a></TD>";
echo "</TR>";
mysqli_close($con);
echo "</TABLE><br>";
