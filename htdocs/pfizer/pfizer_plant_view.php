...
<?php
  ...
  $sql = "SELECT * FROM `pfizer_plant_count`";
  ...
  $sql = "SELECT * FROM `covid-19`.`plant` WHERE `company` = '화이자' ORDER BY `plant code` ASC LIMIT $page_start, $list";
  $ret = mysqli_query($con, $sql);
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='pfizer_plant_view.php?page=1'> 처음 </a>";
    echo "<a href='pfizer_plant_view.php?page=$pre'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'pfizer_plant_view.php?page=$i'> $i </a>";
      }
    }
    echo "<a href='pfizer_plant_view.php?page=$next'> 다음 </a>";
    echo "<a href='pfizer_plant_view.php?page=$total_page'> 마지막 </a>";
