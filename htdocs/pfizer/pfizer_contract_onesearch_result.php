...
<?php
  ...
  if($j == 5)
  {
    ...
    echo "<a href= 'pfizer_contract_search.php'> 되돌아가기 </a>";
    exit();
  }
...
<div style="position: absolute; top:8%; left:91%;"><button onclick="location.href='pfizer_contract_add.php'" style="font-size:15px;width:100px;height:45%">계약추가</button></div>
<div style="position: absolute; top:8%; left:82%;"><button onclick="location.href='pfizer_contract_search.php'" style="font-size:15px;width:100px;height:45%">계약검색</button></div>
...
<?php
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `contract num` = $search_array[0] AND `supplies` = '화이자' ";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `date` = '$search_array[1]' AND `supplies` = '화이자'";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `quantity` = $search_array[2] AND `supplies` = '화이자'";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `unit price` = $search_array[3] AND `supplies` = '화이자'";
        break;        
    case 4:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `due date` = '$search_array[4]' AND `supplies` = '화이자'";
        break; 
  } 
  ...
  {
    case 0:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `contract num` = $search_array[0] AND `supplies` = '화이자' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;
    case 1:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `date` = '$search_array[1]' AND `supplies` = '화이자' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;
    case 2:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `quantity` = $search_array[2] AND `supplies` = '화이자' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;    
    case 3:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `unit price` = $search_array[3] AND `supplies` = '화이자' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break;        
    case 4:
        $sql = "SELECT * FROM `covid-19`.`contract` WHERE `due date` = '$search_array[4]' AND `supplies` = '화이자' ORDER BY `contract num` DESC LIMIT $page_start, $list";
        break; 
  } 
  $ret = mysqli_query($con, $sql);
  mysqli_close($con);
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='pfizer_contract_onesearch_result.php?page=1&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 처음 </a>";
    echo "<a href='pfizer_contract_onesearch_result.php?page=$pre&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'pfizer_contract_onesearch_result.php?page=$i&arr[0]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> $i </a>";
        
      }
    }
    echo "<a href='pfizer_contract_onesearch_result.php?page=$next&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 다음 </a>";
    echo "<a href='pfizer_contract_onesearch_result.php?page=$total_page&arr[]=$search_array[0]&arr[]=$search_array[1]&arr[]=$search_array[2]&arr[]=$search_array[3]&arr[]=$search_array[4]&j=$j'> 마지막 </a>";
