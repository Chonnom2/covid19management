...
$sql ="select *from `confirmed case`, `hospital room` where '".$_POST['room_num']."' = `hospital room`.`room num` AND `confirmed case`.`confirmed num` = '".$_POST['confirmed_num']."' AND `hospital room`.hospital_code='ynuh'";
...
if(!$ret)
{
   echo "데이터 조회 실패!!!"."<br>";
   echo "실패 원인 :".mysqli_error($con);
   echo "<br><a href='Yinsertconfirmed_case.php'><--이전 화면</a>";
   exit();
}
$row = mysqli_fetch_array($ret);
$type = $row['type'];
...
if($ret)
{
  ...
  {
      case 0:
	      $sql ="select * from `confirmed case`, `hospital room` where '".$_POST["room_num"]."' = `hospital room`.`room num` AND `hospital room`.hospital_code='ynuh'";
      	$ret = mysqli_query($con,$sql);
	      $row = mysqli_fetch_array($ret);
