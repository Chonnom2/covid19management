...
?>
  <div id = "search_box" style = "text-align: right">
    <form style = "display: inline" action = "Dconfirmedcase_all.php" method="get" value = <?php echo $search ?>>
    <select name = "catgo">
    ...
    <button onclick="location.href='Dinsertconfirmed_case.php'" style="font-size:14px;width:6%;height:50%"/>환자추가</button></div>
  </div>
<?php
  if(empty($search)) {
  $sql = "SELECT COUNT(*) FROM `confirmed case`";
...
$sql1 = "SELECT `confirmed num`, `name`, `RRN_confirmed`, A.`room num`, `hospitalization date`, `discharge date`, `death date`, `severity` FROM `hospital room` AS ROOM JOIN (SELECT `confirmed num`, `name`, `RRN_confirmed`, `room num`, `hospitalization date`, `discharge date`, `death date`,`severity` FROM `confirmed case` JOIN `citizen` ON `citizen`.`RRN` = `confirmed case`.`RRN_confirmed`)  AS A ON A.`room num` = ROOM.`room num` WHERE ROOM.`hospital_code` = 'dgh' AND $catgo like '%$search%' ORDER BY `confirmed num` LIMIT $page_start, $list";
...
if($row['room num']>=21&&$row['room num']<=30){
    if($row['severity'] == 1)
    {
          $severity = "무증상";
    }
    ...
    echo "<TD><center>", "<a href='Dupdateconfirmed.php?RRN_confirmed=", $row['RRN_confirmed'], "'>수정</a></TD>";
    echo "<TD><center>", "<a href='Ddeleteconfirmed.php?RRN_confirmed=", $row['RRN_confirmed'], "'>삭제</a></TD>";
    echo "</TR>";
    $brcount++;
    }
}
...
<?php
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='Dconfirmedcase_all.php?page=1&catgo=$catgo&search=$search'> 처음 </a>";
    echo "<a href='Dconfirmedcase_all.php?page=$pre&catgo=$catgo&search=$search'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'Dconfirmedcase_all.php?page=$i&catgo=$catgo&search=$search'> $i </a>";
      }
    }
    echo "<a href='Dconfirmedcase_all.php?page=$next&catgo=$catgo&search=$search'> 다음 </a>";
    echo "<a href='Dconfirmedcase_all.php?page=$total_page&catgo=$catgo&search=$search'> 마지막 </a>";
