...
?>
  <div id = "search_box" style = "text-align: right">
    <form style = "display: inline" action = "Ktestcase_all.php" method="get" value = <?php echo $search ?>>
    <select name = "catgo">
    ...
    <button onclick="location.href='Ktestinginsert.php'" style="font-size:14px;width:6%;height:50%"/>검사추가</button></div>
  </div>
<?php
  if(empty($search)) {
  $sql = "SELECT COUNT(*) FROM `testing` WHERE `test location` = 'kbuh'";// 뷰테이블로 구성
  $total = $con->query($sql)->fetch_row()[0];
  }
  else {
    $sql = "SELECT COUNT(*) FROM `testing` WHERE `test location` = 'kbuh' AND $catgo like '%$search%' ";
    $total = $con->query($sql)->fetch_row()[0];
}
  $list = 30;
  $block_cnt = 10;
  ...
  $sql1 = "SELECT `test num`, `name`, `RRN_test`, `test result`, `test date`, `imported case` FROM `testing` JOIN `citizen` ON citizen.RRN = `testing`.`RRN_test` WHERE `test location` = 'kbuh' AND $catgo like '%$search%' ORDER BY `test num` LIMIT $page_start, $list";
...
<?php
  ...
  while($row = mysqli_fetch_array($ret1)) {
    ...
    if($row['imported case'] == 1)
        ...
        echo"<TD><center>","X","</TD>";
    echo "<TD><center>", "<a href='Ktestingupdate.php?test_num=", $row['test num'], "'>수정</a></TD>";
    echo "<TD><center>", "<a href='Ktestingdelete.php?test_num=", $row['test num'], "'>삭제</a></TD>";
    echo "</TR>";
    $brcount++;
}
...
<?php
  ...
  if ($page < $total_page) {$next = $page + 1;}
    ...
    echo "<a href='Ktestcase_all.php?page=1&catgo=$catgo&search=$search'> 처음 </a>";
    echo "<a href='Ktestcase_all.php?page=$pre&catgo=$catgo&search=$search'> 이전 </a>";
    for ($i = $block_start; $i <= $block_end; $i++) {
      if($page == $i) {
        echo "</b> $i </b>";
      } 
      else {
        echo "<a href = 'Ktestcase_all.php?page=$i&catgo=$catgo&search=$search'> $i </a>";
      }
    }
    echo "<a href='Ktestcase_all.php?page=$next&catgo=$catgo&search=$search'> 다음 </a>";
    echo "<a href='Ktestcase_all.php?page=$total_page&catgo=$catgo&search=$search'> 마지막 </a>";
