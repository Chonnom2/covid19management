...
$sql = "SELECT * FROM `hospital room` WHERE `hospital_code`='kbuh'";
...
$sql = "SELECT * FROM `covid-19`.`hospital room` JOIN `covid-19`.`confirmed case` ON `hospital room`.`room num` = `confirmed case`.`room num` WHERE `hospital_code` = 'kbuh'";
...
?>
    <br>
    <div class="card" style="width:24rem;font-size:25px;">
      <div class="card-body">
      경북대병원 코로나 환자 현황
      </div>
      </div>
